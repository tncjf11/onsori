...
export default function AdminMessageEditScreen(){
 ...
 const targetMessageId=messageId||passedItem.messageId||null;
 const targetTranscriptId=transcriptId||passedItem.transcriptId||(!targetMessageId?passedItem.id:null);
 ...
 const getTextValue=(source)=>{
  return source?.content||source?.text||source?.messageText||source?.message||source?.afterText||source?.afterContent||"";
 };

 const getOriginalValue=(source)=>{
  return source?.originalContent||source?.originalText||source?.beforeContent||source?.beforeText||"";
 };
 ...
 const buildFallbackHistory=(source=passedItem)=>{
  const beforeText=getOriginalValue(source)||originalText;
  const currentText=getTextValue(source)||editedText;
  if(beforeText&&currentText&&beforeText!==currentText){
   return[{
    id:source.messageId||source.transcriptId||source.id||`fallback-history-${Date.now()}`,
    beforeText,
    afterText:currentText,
    editedAt:source.updatedAt||source.editedAt||source.createdAt||new Date().toISOString()
   }];
  }
  return[];
 };
 ...
 const fetchHistories=async(token)=>{
  ...
  try{
   logMessageEdit("수정 이력 조회 요청",{transcriptId:targetTranscriptId});
   ...
   const histories=Array.isArray(historyResponse.data?.data)?historyResponse.data.data:[];
 ...
 const handleModifyMessage=async()=>{
  ...
  try{
   ...
   if(isConversationMessage){
    ...
    const nextEditedText=updatedData?.content||updatedData?.text||editedText.trim();
    const nextOriginalText=updatedData?.originalContent||updatedData?.originalText||previousText||getInitialOriginalText();
    ...
    setHistoryList(prev=>{
     const newHistory={
      id:updatedData?.messageId||targetMessageId||`history-${Date.now()}`,
      beforeText:previousText,
      afterText:nextEditedText,
      editedAt:updatedData?.updatedAt||updatedData?.editedAt||new Date().toISOString()
     };
 ...
 const getHistoryMainText=(history)=>{
  return history.afterText||history.text||history.content||history.afterContent||"수정된 내용 없음";
 };

 const getHistorySubText=(history)=>{
  return history.beforeText||history.previousText||history.originalText||history.originalContent||history.beforeContent||"";
 };

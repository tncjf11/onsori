...
import{ActivityIndicator,Alert,Keyboard,KeyboardAvoidingView,Platform,ScrollView,TouchableOpacity,View}from"react-native";
...
const CLOSED_SESSION_STATUSES=new Set(["CLOSED","ENDED","COMPLETE","COMPLETED","FINISHED","SUCCESS","FAILED","MISSED","NO_ANSWER","CANCELED","CANCELLED"]);
...
export default function AdminMonitoringDetailScreen(){
 ...
 const targetSessionId=sessionId??passedItem?.sessionId??passedItem?.id??null;
 ...
 const getMessageId=(msg,idx)=>{
  return msg.messageId??msg.transcriptId??msg.id??msg.chunkOrder??`msg-${idx}`;
 };

 const getMessageText=msg=>{
  return msg.content||msg.text||msg.messageText||msg.message||msg.rawText||"";
 };

 const getOriginalText=msg=>{
  return msg.originalContent||msg.originalText||msg.beforeContent||msg.beforeText||"";
 };
 ...
 const normalizeMessages=messages=>{
  ...
  messages.forEach((msg,idx)=>{
   ...
   if(uniqueMap.has(String(key)))return;

   uniqueMap.set(String(key),{
    ...msg,
    id,
    text,
 ...
 const logMessageUpdateIfChanged=(messages,nextSessionInfo=sessionInfo)=>{
  const signature=messages.map((msg,idx)=>{
   const id=getMessageId(msg,idx);
  ...
  logMonitoringDetail("메시지 갱신",{
   ...
   lastMessageId:lastMessage?getMessageId(lastMessage,messages.length-1):null,
 ...
 const stopRealtimeSubscription=()=>{
  ...
  try{
   ...
   logMonitoringDetail("실시간 STOMP 구독 해제 실패",error?.message);
  }
 ...
 const commitRealtimePartial=text=>{
  ...
  if(!safeText||!isMountedRef.current)return;

  const localId=`realtime-admin-${targetSessionId}-${Date.now()}-${++realtimeUtteranceSeqRef.current}`;
 ...
 const handleRealtimeTranscript=(sessionId,payload)=>{
  ...
  const realtimeType=String(payload.type||"").trim().toLowerCase();

  if(
   realtimeType&&
   realtimeType!=="partial"&&
   realtimeType!=="final"
  ){
   return;
  }
  ...
  if(realtimeType==="final"){
   const finalMessage=normalizeMessages([payload])[0];

   if(finalMessage){
    setChatMessages(prev=>{
     const messageId=finalMessage.messageId||finalMessage.id;
     ...
     const realtimeIndex=prev.findIndex(
      item=>
       item.__realtimeOnly===true&&
   ...
   setTimeout(()=>{
    if(isMountedRef.current)scrollChatToBottom(true);
   },50);
 ...
 const handleRealtimeConversationMessage=(sessionId,payload)=>{
  ...
  if(messageId==null){
   logMonitoringDetail("messageId 없는 보조 메시지 이벤트 무시",payload);
   return;
  }
  ...
  if(!incoming||!isMountedRef.current)return;
 ...
 const startRealtimeSubscription=sessionId=>{
  ...
  if(!sessionId||endedSessionRef.current)return;
  ...
  navigation.navigate(
   "AdminMessageEdit",
   {
    ...
    item:{
     ...
     originalContent:getOriginalText(
      selectedMessage
     ),
